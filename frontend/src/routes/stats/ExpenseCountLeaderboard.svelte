<script lang="ts">
	...
	let leaderboard: Promise<PaginatedResponse<LeaderboardEntry>> = $derived(
		api.stats.leaderboard(1, 20, rangeStart(range), undefined, '-expense_count')
	);
</script>
			...
			<tbody>
				{#each res.data as entry, idx}
					...
					>
						...
						<td class="px-4 py-3 text-right align-middle tabular-nums">{entry.expense_count}</td>
					</tr>
				{/each}
			</tbody>
		</table>
