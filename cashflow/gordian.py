...
# Helper functions
# ======================
def _build_cache_key(name: str) -> str:
...
def find_cost_center(cc_id: int = None, name: str = None, force_refresh=False) -> GCostCenter:
    ...
    if cc_id is not None:
        try:
            return next(cc for cc in all_cost_centers if cc.id == cc_id)
        except StopIteration as e:
            raise ValueError(f"Couldn't find cost center with {cc_id=}") from e
    elif name is not None:
        cost_center = cache.get(f"gordian:cost_center:search:name:{_build_cache_key(name)}", None)
        if cost_center is not None:
            logger.debug("Cache hit for cost center with name %s", name)
            return GCostCenter.model_validate(cost_center, by_name=True)
        try:
            cost_center = next(cc for cc in all_cost_centers if cc.name == name)
            cache.set(f"gordian:cost_center:search:name:{_build_cache_key(cost_center.name)}", cost_center.model_dump(),
...
def find_snd_cost_center(scc_id: int = None, name: str = None, force_refresh=False) -> GSecondaryCostCenter:
    ...
    if scc_id is not None:
        try:
            return next(scc for scc in all_snd_cost_centers if scc.id == scc_id)
        except StopIteration as e:
            raise ValueError(f"Couldn't find cost secondary center with {scc_id=}") from e
    elif name is not None:
        secondary_cost_center = cache.get(f"gordian:secondary_cost_center:search:name:{_build_cache_key(name)}", None)
        if secondary_cost_center is not None:
        ...
        try:
            ...
            cache.set(f"gordian:secondary_cost_center:search:name:{_build_cache_key(secondary_cost_center.name)}", secondary_cost_center.model_dump(),
                      timeout=settings.GORDIAN_COST_CENTER_CACHE_TIMEOUT * 60 * 60)
        except StopIteration as e:
            raise ValueError(f"Couldn't find secondary cost center with {name=}") from e
        return secondary_cost_center
    else:
...
def find_budget_line(bl_id: int = None, name: str = None, force_refresh=False) -> GBudgetLine:
    ...
    if bl_id is not None:
        try:
            return next(bl for bl in all_budget_lines if bl.id == bl_id)
        except StopIteration as e:
            raise ValueError(f"Couldn't find cost budget line with {bl_id=}") from e
    elif name is not None:
        budget_line = cache.get(f"gordian:budget_line:search:name:{_build_cache_key(name)}", None)
        if budget_line is not None:
            logger.debug("Cache hit for budget line with name %s", name)
            return GBudgetLine.model_validate(budget_line, by_name=True)
        try:
            budget_line = next(bl for bl in all_budget_lines if bl.name == name)
            cache.set(f"gordian:budget_line:search:name:{_build_cache_key(budget_line.name)}", budget_line.model_dump(),
                      timeout=settings.GORDIAN_COST_CENTER_CACHE_TIMEOUT * 60 * 60)
        except StopIteration as e:
...
def list_secondary_cost_centres_from_gordian(cost_center: Union[int, GCostCenter] = None,
    ...
    if cost_center is not None:
        ...
        def _fetch_sccs(cc):
            response = requests.get("https://budget.datasektionen.se/api/SecondaryCostCentres", params={"id": cc.id})
            return SCC_LIST.validate_json(response.text)

        with ThreadPoolExecutor() as executor:
            results = list(executor.map(_fetch_sccs, cost_centers))
        secondary_cost_centers = [scc for batch in results for scc in batch]
...
def list_budget_lines_from_gordian(secondary_cost_center: Union[int, GSecondaryCostCenter] = None,
    ...
    if secondary_cost_center is not None:
        ...
        def _fetch_bls(scc):
            resp = requests.get("https://budget.datasektionen.se/api/BudgetLines", params={"id": scc.id})
            if resp.text.strip() == "null":
                return []
            return BL_LIST.validate_json(resp.text)

        with ThreadPoolExecutor() as executor:
            results = list(executor.map(_fetch_bls, secondary_cost_centers))
        budget_lines = [bl for batch in results for bl in batch]
...
def get_budgetline_snapshot(bl_id: int):
    """Looks up a GOrdian budget line by ID and returns a Budgetline snapshot object."""
    from cashflow import snapshots
    bl = find_budget_line(bl_id=bl_id)
    scc = find_snd_cost_center(scc_id=bl.scc_id)
    cc = find_cost_center(cc_id=scc.cc_id)
    return snapshots.Budgetline(name=bl.name, cost_center=cc.name, secondary_cost_center=scc.name)
