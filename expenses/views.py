...
def edit_expense(request, pk):
    ...
    for cost_centre, secondary_cost_centre, budget_line, amount in zip(
        ...
        if float(amount) < 1:
            messages.error(
                request,
                "En budgetpost innehåller en felaktig summa och kunde därför inte sparas",
            )
            continue
...
def delete_expense(request, pk):
    ...
    if expense.reimbursement is not None:
        return HttpResponseBadRequest(
            "Du kan inte ta bort ett kvitto som är återbetalt!"
        )
...
def new_payment(request):
    ...
    if (
        expense_owner.bank_name == ""
        or expense_owner.bank_account == ""
        or expense_owner.sorting_number == ""
    ):
        return HttpResponseBadRequest(
            "Användaren har inte angett alla sina bankuppgifter"
        )

    payment = models.Payment(payer=request.user.profile, receiver=expense_owner)
    payment.save()
