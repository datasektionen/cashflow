...
def list_verification(request):
    ...
    def apply_verification_filters(queryset):
        queryset = (
            queryset.exclude(verification__isnull=True)
            .annotate(
            ...
            .annotate(
                verification_priority=Case(
                    When(
                        verification_compact__regex=r"^[EU][0-9]+$", then=Value(1)
                    ),
                    default=Value(0),
                ...
                verification_number=Case(
                    When(
                        verification_compact__regex=r"^[EU][0-9]+$",
                        then=Cast(Substr("verification_compact", 2), IntegerField()),
                    ),
                    default=Value(-1),
                    output_field=IntegerField(),
                ),
        ...
        verification_list.append(
            {
                ...
                "owner_href": reverse(
                    "user-show", kwargs={"username": expense.owner.user.username}
                ),
    ...
    for invoice in apply_verification_filters(
        Invoice.objects.annotate(
            verification_date=Coalesce("invoice_date", "created_date")
        ).filter(verification_date__year=year)
    ):
        verification_list.append(
            {
                "id": invoice.id,
                ...
                "owner_href": reverse(
                    "user-show", kwargs={"username": invoice.owner.user.username}
                ),
