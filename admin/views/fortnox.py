...
client = FortnoxAPIClient(client_id=settings.FORTNOX_CLIENT_ID, client_secret=settings.FORTNOX_CLIENT_SECRET,
    scope="bookkeeping%20companyinformation%20settings%20customer%20profile", state=settings.FORTNOX_STATE, )
...
def check_or_update_token(user):
    ...
    try:
        ...
        try:
            ...
            APITokens.objects.update_or_create(user=user,
...
def auth_complete(request):
    ...
    match request.GET.get('code'), request.GET.get('error'):
        ...
        case auth_code, _:
            ...
            APITokens.objects.update_or_create(user=request.user,
                defaults={'access_token': response.access_token, 'refresh_token': response.refresh_token, })
    ...
    while True:
        ...
        for account in accounts['Accounts']:
            account_db = Account(URL="None" if account.get('@url') is None else account.get('@url'),
                ...
                BalanceBroughtForward=0 if account.get('BalanceBroughtForward') is None else account.get(
                    'BalanceBroughtForward'),
                CostCenter="None" if account.get('CostCenter') is None else account.get('CostCenter'),
                CostCenterSettings="None" if account.get('CostCenterSettings') is None else account.get(
                    'CostCenterSettings'),
