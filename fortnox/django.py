...
class FortnoxServiceMiddleware:
    ...
    def __call__(self, request):
        ...
        allowed = not request.user.is_anonymous and (
            self.auth_callback is None or self.auth_callback(request.user)
        )
    ...
    with transaction.atomic():
        ...
        if service_account.expires_at > timezone.now():
            ...
            try:
                ...
                service_account.expires_at = timezone.now() + timedelta(
                ...
                logger.info(
                    f"refreshed access token for Fortnox",
                    new_token_id=service_account.pk,
